...
// 👍 48 👎 0
public class No1418_displayTable {
    public static void main(String[] args) {
        String[][] p = new String[][]{
                {"David", "3", "Ceviche"}, {"Corina", "10", "Beef Burrito"}, {"David",
                "3", "Fried Chicken"}, {"Carla", "5", "Water"}, {"Carla", "5", "Ceviche"}, {"Rous", "3", "Ceviche"}
        };
        List<List<String>> list = Arrays.stream(p).map(q -> Arrays.stream(q).collect(Collectors.toList())).collect(Collectors.toList());
        System.out.println(new No1418_displayTable().displayTable(list));
    }
    ...
    public List<List<String>> displayTable(List<List<String>> orders) {
        ...
        firstRow.add("Table");
        res.add(firstRow);
        Map<String, List<List<String>>> table = orders.stream().collect(Collectors.groupingBy(p -> p.get(1)));
        List<String> foods = orders.stream().map(p -> p.get(2)).distinct().sorted().collect(Collectors.toList());
        firstRow.addAll(foods);
        List<String> tableNos = new ArrayList<>(table.keySet());
        tableNos.sort(Comparator.comparingInt(Integer::parseInt));
        for (String tableNo : tableNos) {
            List<List<String>> tableOrders = table.get(tableNo);
            Map<String, List<List<String>>> foodMap = tableOrders.stream().collect(Collectors.groupingBy(p -> p.get(2)));
            List<String> curRow = new ArrayList<>();
            curRow.add(tableNo);
            for (String food : foods) {
                List<List<String>> list = foodMap.get(food);
                int num = list == null ? 0 : list.size();
                curRow.add(String.valueOf(num));
            }
            res.add(curRow);
        }
        return res;
