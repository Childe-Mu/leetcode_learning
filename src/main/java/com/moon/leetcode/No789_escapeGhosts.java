 ...
 * </ul>
 * <div><div>Related Topics</div><div><li>数组</li><li>数学</li></div></div><br><div><li>👍 87</li><li>👎 0</li></div>
 */
public class No789_escapeGhosts {
    public boolean escapeGhosts(int[][] ghosts, int[] target) {
        ToIntFunction<int[]> f = o -> Math.abs(o[0] - target[0]) + Math.abs(o[1] - target[1]);
        int distance = f.applyAsInt(new int[]{0, 0});
        for (int[] ghost : ghosts) {
            if (distance >= f.applyAsInt(ghost)) {
                return false;
            }
        }
        return true;
