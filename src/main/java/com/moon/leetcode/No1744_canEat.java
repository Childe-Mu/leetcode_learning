...
// Related Topics 数学
// 👍 76 👎 0
public class No1744_canEat {
    public static void main(String[] args) {
        System.out.println(Arrays.toString(new No1744_canEat().canEat(new int[]{5, 2, 6, 4, 1}, new int[][]{{3, 1, 2}, {4, 10, 3}, {3, 10, 100}, {4, 100, 30}, {1, 3, 1}})));
    }
    ...
    public boolean[] canEat(int[] candiesCount, int[][] queries) {
        ...
        for (int i = 0; i < n; i++) {
            sum[i + 1] = (long) candiesCount[i] + sum[i];
        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            int cap = queries[i][2];
//            ans[i] = !((long) cap * day < sum[type] + 1 ||  day > sum[type + 1]);
            ans[i] = sum[type] / cap + 1 <= day && day <= sum[type + 1];
        }
        return ans;
    }
}
